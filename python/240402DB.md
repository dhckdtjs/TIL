...
* 카카오 - 제1데이터센터와 제2데이터세터에 1.5조 투자 (2022)
...
데이터베이스에 정보를 저장하고 처리하기 위한 프로그래밍 언어 (테이블의 형태로 구조화된 관계형 데이터베이스에게 요청을 질의(요청))
### SQL Syntax
...
-- 같은 이름의 table이 존재한다면 error 발생
-- AUTOINCREMENT : 자동으로 하나씩 증가, 재사용 X
-- NOT NULL : NULL 값 들어오지 않음
```
### PRAGMA
* 테이블 schema(구조) 확인
...
CREATE TABLE articles (
  id INTEGER PRIMARY KEY AOUTOINCREMENT,
